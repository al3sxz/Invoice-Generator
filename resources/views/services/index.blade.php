...
@endif
    ...
    <div class="flex justify-between nx-table-toolbar" >
     ...
     <div style="position:relative;flex:1;max-width:260px">
        ...
        <input class="nx-search" name="service" id="nx-search" placeholder="Search...">
      </div>
      </form>
      <div class="flex gap-6 items-center ml-auto">
        <a href="/dashboard/services/create" class="nx-btn btn-primary">
      ...
      <table>
        <thead>
          <tr>
            <th class="sortable"  id="th-name">
              Service Name<span class="sort-icon"></span>
            </th>
            <th>Description</th>
            <th class="sortable"  id="th-precio">
        ...
        <tbody id="nx-tbody">
          @foreach ($services as $service)
           <tr>
        <td><h2 class="cell-name">{{ $service->service_name }}</h2></td>
        <td><p class="cell-name">{{ $service->description }}</p></td>
        <td><span class="cell-price">{{ $service->price }}</span></td>
        <td>
      
            <span class="badge-inactive"><span class="dot dot-inactive "></span>{{ $service->is_active == 1 ? "ACTIVE" : "INACTIVE" }}</span>
        </td>
        <td style="text-align:right">
          <div style="display:inline-flex;gap:6px">
             <a href="/dashboard/services/{{ $service->id }}/edit" class="action-btn" >
